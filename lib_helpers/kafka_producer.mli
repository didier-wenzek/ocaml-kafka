...
val partition_sink :
  ...
  ?delivery_error_handler:
    string push_error_handler (* The default is to raise an error *) ->
  string ->
  partition:Kafka.partition option (* topic name and partition to stream into *) ->
  (string, unit) sink
...
val topic_sink :
  ...
  (Kafka.partition * string, unit) sink
(* stream of (partition,message) pairs *)
