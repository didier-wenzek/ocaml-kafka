...
let tail_topic brokers msg_count timeout_ms topic_name partition offset =
  ...
  let partitions = match partition with
    | Some p -> [p]
    | None -> (Kafka.topic_metadata consumer topic).topic_partitions
  in
  let start () =
    List.iter (fun partition -> Kafka.consume_start_queue queue topic partition offset) partitions
    |> return
  in
  let rec loop () = 
...
let brokers =
  ...
  Arg.(value & opt string "localhost" & info ["b"; "brokers"] ~docv:"BROKERS" ~doc)
...
let timeout_ms =
  let doc = "Wait at most $(docv) milli-seconds to fill a batch of the requested size." in
